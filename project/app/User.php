...
class User extends Authenticatable
{
                ...
                if (count($location_params) > 0)
                {
                    foreach($location_params as $location_key => $region)
                    {
                        ...
                        if ( $region != null)
                        {
                          ...
                          $region_details = Region::updateOrCreate(['region_name' => $region, 'url_slug' => strtolower(str_replace(" ", "-", trim($region)))], ['region_name' => $region, 'url_slug' => strtolower(str_replace(" ", "-", trim($region)))]);
                          if ($user->locations()->count() <= 20 && $user->locations()->where(['region_id' => $region_details->id])->first() == null)
                          {
                            /**Adding member to location**/
                            $user->locations()->create(['region_id' => $region_details->id]);
                            /**Making member to follow to that location**/
                            $region_details->regionFollowers()->updateOrCreate(
                                ['region_id' => $region_details->id, 'user_id' => $user->id],
                                ['region_id' => $region_details->id, 'user_id' => $user->id]);
                          }
                        }
                    }
    ...
    public function userFollowedFriends()
    {
        return $this->userFriends()->where('is_followed', '=', 1)->where('is_blocked', '=', 0);
    }
